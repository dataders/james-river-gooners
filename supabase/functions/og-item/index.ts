...
// visitors are immediately redirected to the SPA deep-link URL.
//
...
function buildHtml(lot: LotRow, spaUrl: string): string {
...
Deno.serve(async (req) => {
  ...
  if (!itemParam || colon < 1) {
    return Response.redirect(SPA_ORIGIN, 302)
  }

  const auctionSafeId = itemParam.slice(0, colon)
  const itemId = itemParam.slice(colon + 1)
  const spaUrl = `${SPA_ORIGIN}?item=${encodeURIComponent(itemParam)}`
  ...
  try {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/lots?${qs}`, {
      headers: {
  ...
  return new Response(buildHtml(lot, spaUrl), {
    headers: {

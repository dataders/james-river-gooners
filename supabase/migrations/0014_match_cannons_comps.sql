-- Cannon's comps over Nomic pgvector (CLIP→Nomic migration, phase 3).
--
-- For every embedded active item in `active_auction`, return its top-K most
-- similar *sold* archived lots (cosine over nomic_embeddings, joined to sold_lots
-- for the realized price + display fields), excluding the item's own auction.
-- This replaces cannons_comps.py's in-process CLIP corpus + dot-product loop:
-- the vector search now runs in Postgres against the HNSW index, so the comps
-- job needs no model and no committed .embeddings sidecars.
--
-- SECURITY DEFINER so the scraper (and authenticated members) can read across the
-- RLS-locked nomic_embeddings + sold_lots on the caller's behalf; it returns only
-- sold-lot facts that are already members-visible via public_sold_lots.

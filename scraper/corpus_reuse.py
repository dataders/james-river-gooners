...
def has_fresh_coverage(matches: list[dict], *, min_fresh: int = _MIN_FRESH,
                       max_age_days: int = _MAX_AGE_DAYS, now=None) -> bool:
...
def reuse_comp_rows(matches: list[dict], safe_id: str, item_id: str, fetched_at: str) -> list[dict]:
...
def fetch_item_coverage(safe_id: str, item_id: str, url: str, key: str, session,
    ...
    resp = _request_with_retry(
        partial(
            session.post, endpoint, headers=headers,
            data=json.dumps({
...
class CorpusReuser:
    ...
    def covered_comps(self, item: dict) -> list[dict] | None:
        ...
        try:
            matches = fetch_item_coverage(safe_id, item_id, self.url, self.key, self._session_obj())
        except RuntimeError:
            return None  # coverage check must never break the fetch — just spend the API

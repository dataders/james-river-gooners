...
def load_sample(session, url, key, n):
    ...
    embs = _get(
        ...
        key,
        NOMIC_TABLE,
        {"select": "auction_safe_id,item_id,embedding", "item_id": in_clause},
    )
    emb_by = {
        (e["auction_safe_id"], e["item_id"]): _parse_vec(e["embedding"]) for e in embs
    }
    lots = _get(
        session,
        url,
        key,
        LOTS_TABLE,
        {"select": "auction_safe_id,item_id,title,description", "item_id": in_clause},
    )
...
def arm_search(item, arm):
    title_q = eq.item_exact_phrase(item) or " ".join(
        eq.meaningful_tokens(eq.compact_item_text(item))[:5]
    )
...
def candidate_vectors(session, url, key, candidates):
    ...
    by_id = {
        c["ebay_item_id"]: c
        for c in candidates
        if c.get("ebay_item_id") and c.get("thumbnail_url")
    }
    vecs = {}
    ids = list(by_id)
    for start in range(0, len(ids), 100):
        ...
        rows = _get(
            ...
            key,
            EMB_TABLE,
            {"select": "ebay_item_id,embedding", "ebay_item_id": in_clause},
        )
        for r in rows:
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="A/B comp-quality eval (Nomic similarity)"
    )
    ...
    for n, item in enumerate(sample, 1):
        ...
        for arm in ("baseline", "filters", "enrichment"):
            result = soldcomps_sold_matches(
                session, arm_search(item, arm), api_key=api_key, max_matches=_COUNT
            )
        ...
        for rank, (c, sim) in enumerate(rr, 1):
            eval_rows.append(
                (
                    ...
                    rank,
                    sim,
                    arm_search(item, "enrichment")["query"],
                    c.get("title"),
                    c.get("price_value"),
                    c.get("sold_date"),
                )
        ...
        if n % 10 == 0:
            print(
                f"  [{n}/{len(sample)}] "
                + " ".join(f"{a}={np.mean(v):.3f}" for a, v in arm_sims.items() if v)
            )
    ...
    for start in range(0, len(payload), 500):
        _request_with_retry(
            partial(
                ...
                data=json.dumps(payload[start : start + 500]),
                timeout=(10, 60),
            ),
            "store eval rows",
        )

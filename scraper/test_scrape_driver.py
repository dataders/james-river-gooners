...
class JobBuilderTest(unittest.TestCase):
    def test_maxanet_job(self):
        ...
        self.assertEqual(job.cmd, [sys.executable, "scrape.py", "https://bid.cannonsauctions.com/x?AuctionId=abc"])
...
class ScrapeSourceTest(unittest.TestCase):
    def test_collects_failures_by_fail_id(self):
        ...
        with mock.patch.object(rescrape_all, "_run_with_retry", side_effect=[0, 1]):
            failures = _scrape_source(
                HIBID,
                [
                    {"catalog_url": "https://hibid.com/catalog/1/", "source_slug": "s", "company_name": "C"},
                    {"catalog_url": "https://hibid.com/catalog/2/", "source_slug": "s", "company_name": "C"},
                ],
                total=2,
                start_i=1,
            )
    ...
    def test_runs_each_spec_once_with_built_command(self):
        calls = []
        with mock.patch.object(rescrape_all, "_run_with_retry", side_effect=lambda cmd, cwd, label: calls.append(cmd) or 0):
    ...
    def test_runner_names(self):
        self.assertEqual((MAXANET.name, HIBID.name, RASMUS.name), ("Maxanet", "HiBid", "Rasmus"))

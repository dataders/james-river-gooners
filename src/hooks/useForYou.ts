...
// pushed away from the centroid of their "not interested" list — Rocchio feedback).
//
// Only runs when enabled=true (i.e. sort==='foryou' AND the user has history).
// Re-fetches when the history set, ignored set, or target auction set changes.
  ...
  useEffect(() => {
    ...
    void (async () => {
      const { data, error } = await supabase.rpc('rank_for_you', {
        ...
        ignored_auction_ids: ignoredAuctionIds,

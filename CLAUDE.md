...
**Changelog / What's New** (`src/data/changelog.js`, `src/components/WhatsNewModal.jsx`, `CHANGELOG.md`) — user-facing release notes. `src/data/changelog.js` is the **single source of truth** (newest entry first; each a dated group of plain-language change lines, written for someone browsing auctions, not implementation detail); it feeds both the in-app ✨ "What's New" panel and `CHANGELOG.md`, which must stay in sync. `useWhatsNew` tracks what each visitor has already seen via the `gooners-whatsnew-seen` localStorage key (the `date` of the newest entry the last time they opened the panel): the header ✨ button shows an "unseen" dot, and the panel badges any release newer than that date as **New**. **Keep this current** — whenever you ship something a user would notice (a feature, a visible behavior change), add a dated entry at the top of `changelog.js` and mirror the same lines into `CHANGELOG.md`. Bump/add the date so the unseen dot fires for returning users; don't log internal-only refactors, data refreshes, or scraper plumbing.

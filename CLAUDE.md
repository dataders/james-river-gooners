...
- Nomic embeddings (#165): `GOONERS_NOMIC_EMBEDDINGS=1` + `SUPABASE_SECRET_KEY` triggers `embed_nomic.py` (text+vision, 768-dim → Supabase pgvector `nomic_embeddings`, upsert by `(auction_safe_id, item_id)`). **Also incremental** — `maybe_generate_and_upsert` reads the auction's already-embedded item_ids and embeds only the new ones, so the two ~550 MB models load only when there's new work. Requires `sentence-transformers` + `pillow` + **`einops`** (nomic_bert's `trust_remote_code` modeling file imports it; without `einops` the model fails to load and every upsert is silently skipped) + **`transformers==4.49.0`** (transformers 5.x's strict config validation rejects nomic_bert's remote config — `n_inner=2048.0` float — so the vision model won't load on the latest release) + **`torchvision`** (the vision model's `AutoImageProcessor`). The vision model is loaded via transformers `AutoModel`/`AutoImageProcessor` (CLS token of `last_hidden_state`, L2-normalised) — *not* `SentenceTransformer`, which would build a text tower (vocab_size 0) and crash. These were the bugs that left the table empty.

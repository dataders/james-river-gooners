...
def test_public_get_sends_apikey_and_pub_bearer():
    c = _client()
    with patch(
        "gooners_mcp.client.requests.request", return_value=_resp(200, [{"x": 1}])
    ) as req:
...
def test_edge_fn_posts_to_functions_path():
    c = _client()
    with patch(
        "gooners_mcp.client.requests.request",
        return_value=_resp(200, {"ids": ["a:1", "b:2"]}),
    ) as req:

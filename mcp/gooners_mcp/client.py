...
class GoonersClient:
    ...
    def _headers(self, auth: bool, prefer: str | None = None) -> dict:
        h = {
            "apikey": self._key,
            "Authorization": f"Bearer {self._bearer(auth)}",
    ...
    def _send(
        self, method: str, url: str, *, auth: bool, params=None, json=None, prefer=None
    ) -> requests.Response:
        last_exc = None
        for attempt in range(_MAX_RETRIES):
            try:
                resp = requests.request(
                    method,
                    url,
                    headers=self._headers(auth, prefer),
                    params=params,
                    json=json,
                    timeout=DEFAULT_TIMEOUT,
                )
            ...
            return resp
        raise RuntimeError(
            f"backend unreachable after {_MAX_RETRIES} tries: {last_exc}"
        )
    ...
    def post(
        self, path: str, json: Any, *, auth: bool = False, prefer: str | None = None
    ) -> Any:
        resp = self._send(
            "POST", f"{self._url}{path}", auth=auth, json=json, prefer=prefer
        )
        resp.raise_for_status()
    ...
    def edge_fn(self, name: str, payload: dict, *, auth: bool = False) -> Any:
        resp = self._send(
            "POST", f"{self._url}/functions/v1/{name}", auth=auth, json=payload
        )
        resp.raise_for_status()
        return resp.json()
